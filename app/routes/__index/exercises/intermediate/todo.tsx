...
type ActionType =
  ...
  | { type: "changeColor"; payload: { id: TaskId; color: TaskColor } }
...
export default function TodoApp() {
  ...
  const [editedTaskId, setEditedTaskId] = React.useState<TaskId | null>(null);
  ...
  if (status !== StatusFilter.ALL || colors.length !== 0 || cleanSearch.length !== 0) {
    ...
    filteredTasks = tasks.filter(task => {
      const statusMatches = status === StatusFilter.ALL || task.done === showCompleted;
      ...
      >
        {filteredTasks.map(task => (
          <TaskItem
            ...
            task={task}
            dispatch={dispatch}
            isEditing={task.id === editedTaskId}
            onEditChange={isEdited => setEditedTaskId(isEdited ? task.id : null)}
          />
        ))}
      </ul>
